...
// Fetch following users ordered by name
$stmt = $con->prepare("
...
<div class="container">
  ...
  <?php if (count($following) === 0): ?>
    <p class="no-following">Not following anyone yet.</p>
  <?php else: ?>
    <ul class="list-group">
        ...
        <li class="list-group-item">
          ...
          <?php if ($row['user_id'] !== $logged_in_user_id): ?>
            <button
              ...
              onclick="toggleFollow(<?php echo $row['user_id']; ?>, this)"
            >
...
function toggleFollow(targetUserId, btn) {
  ...
  fetch('follow_action.php', {
    ...
    body: `target_user_id=${targetUserId}&action=${action}`
  })
  .then(res => res.json())
  .then(data => {
